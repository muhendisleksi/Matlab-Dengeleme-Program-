function triyuksekliknivelmanprint(path)
...
if knan ~= 0
    fprintf(fid,'\t%s\n',datetime('now'));
    fprintf(fid,'\r\n\r\n\t ************** YÜKSEKLÝK FARKINA GÖRE TRÝGONOMETRÝK NÝVELMAN AÐLARININ DENGELENMESÝ **************\r\n');
    fprintf(fid,'\r\n\r\n--------------------------------------------------------------------------------\r\n\r\n \t 1 - VERÝLENLER\r\n');
fprintf(fid,'--------------------------------------------------------------------------------\r\n');
...
fprintf(fid,'----------------------------------------------------------------------\r\n');
fprintf(fid,'\n\t\t **** Hesap Tablosu\n\n');
fprintf(fid,'\t DN\t BN\t    S*CotZij\t      K*Sij^2\t       Hij\t     lik (cc)\n');
fprintf(fid,'\t ==\t ==\t ============\t     ==========\t   =========\t   =========\t \n');
for i = 1:length(YON_BLNM)
    fprintf(fid,'\t %2.d\t %2.d\t %10.3f %16.4f\t %12.5f\t %10.4f\n',[DNdog(DNdog==YON_BLNM(i),1) BNdog(DNdog==YON_BLNM(i),1)  H00{i}  KSS{i} Hijj{i}  lii{i}]');
    fprintf(fid,'\n\n');
end
fprintf(fid,'\n\t#### P AGIRLIK MATRISI\r\n\r\n');
for i = 1:size(P,1)
...
fclose(fid);
    ...
    fprintf(fid,'\t%s\n',datetime('now'));
    fprintf(fid,'\r\n\r\n\t ************** SERBEST YÜKSEKLÝK FARKLARINA GÖRE TRÝGONOMETRÝK NÝVELMAN AÐLARININ DENGELENMESÝ **************\r\n');
    fprintf(fid,'\r\n\r\n--------------------------------------------------------------------------------\r\n\r\n \t 1 - VERÝLENLER\r\n');
    fprintf(fid,'--------------------------------------------------------------------------------\r\n');
    fprintf(fid,'\n\t#### YAKLAÞIK YÜKSEKLÝK \r\n\r\n\t  N.N\t\t H \r\n');
    fprintf(fid,'\t  ==\t      ======== \r\n');
